...
void configure_radio(uint8_t sf_factor, uint16_t band_width)
{
	uint32_t bw = 2; // 500kHz
	if(band_width == 125){
		bw = 0;
	}
	else if(band_width == 250){
		bw = 1;
	}
	else{
		bw = 2;
	...
	Radio.SetTxConfig(MODEM_LORA, TX_OUTPUT_POWER, 0, bw,
					(uint32_t)sf_factor, LORA_CODINGRATE,
					LORA_PREAMBLE_LENGTH, LORA_FIX_LENGTH_PAYLOAD_ON,
					true, 0, 0, LORA_IQ_INVERSION_ON, TX_TIMEOUT_VALUE);
...
void lora_xmit_packet(char* buffer, int size){
